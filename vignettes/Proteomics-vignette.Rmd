...
`omicsGMF` builds on the `sgdGMF` package, but 
provides easy functions for `SingleCellExperiment` and `SummarizedExperiment` 
...
described in our preprint [@Castliglioni2024]. There, we show the use of the
`sgdGMF-framework` on single-cell RNA-seq data. In our other preprint 
[@Segers2025], we show how `omicsGMF` can be used to visualize and impute 
missing values in (single-cell) proteomics data.
...
of latent confounders. These results can be visualized using `plotCV`. This 
avoids arbitrarily choosing `ncomponents`, but
...
This can be done with the `calculateCVGMF` function, which builds on the 
\link{sgdgmf.cv} function from the `sgdGMF` package. Although the `sgdGMF` 
package allows great flexibility regarding the optimization algorithm, 
...
the optimization history of the `sgdGMF` framework and many more are available 
in the `attributes`. See \link{runGMF} for all outputs. 
